  ...
  Spiderable.prototype.middleware = function (req, res, next) {
    ...
    if (urlObj.query._escaped_fragment_ !== void 0 || this.botsRE.test(req.headers['user-agent'] || '')) {
      ...
      var opts  = {
        ...
        headers: {
          'User-Agent': 'spiderable-middleware/1.4.0'
        },
        rawBody: true
      };
      ...
      try {
        ...
        serviceReq.request.on('header', function (header) {
          ...
          if (!!~h.indexOf(':')) {
            ...
            if (h[0] === 'status' && !res.headersSent) {
              res.statusCode = parseInt(h[1].replace(re.nonDigit, ''));
              usedHeaders.push(h[0]);
            } else if (h[1].length && !res.headersSent && !usedHeaders.includes(h[0])) {
