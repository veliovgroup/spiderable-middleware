...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!url.origin.includes('ostr.io') && BOT_AGENTS.includes(userAgent) && !(extension && IGNORE_EXTENSIONS.includes(extension))) {
      ...
      // OPTIONAL BLOCK FOR SUPPORT OF _escaped_fragment_ get query
      // DPRECATED BY GOOGLE: https://developers.google.com/search/blog/2015/10/deprecating-our-ajax-crawling-scheme
      // BUT STILL MIGHT BE SUPPORTED BY OTHER ENGINES
      const escapedFragment = url.searchParams.has('_escaped_fragment_') ? url.searchParams.get('_escaped_fragment_') : false;
      if (typeof escapedFragment === 'string' && escapedFragment.length) {
        url.searchParams.delete('_escaped_fragment_');
        fetchUrl += `${url.pathname.replace(trailingSlashRe, '')}/${escapedFragment.replace(beginningSlashRe, '')}`;
      } else {
        fetchUrl += url.pathname;
      }

      // OPTIONAL BLOCK FOR SUPPORT OF get query
      // COMMENT OUT TO DISABLE MULTIPLE RENDERING OF PAGES WITH ?getquery=
      if (typeof url.search === 'string' && url.search.length > 1) {
        fetchUrl += url.search;
      }

      return fetch(new Request(`https://render.ostr.io/?url=${encodeURIComponent(fetchUrl)}&bot=${encodeURIComponent(userAgent)}`, {
        headers: headers,
        redirect: 'manual',
      }));
    }
